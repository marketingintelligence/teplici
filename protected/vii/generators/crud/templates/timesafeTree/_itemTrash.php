...
?><div class="row">
    ...
    <div class="offset3">
        <p class="pull-right">
            <a class="btn success" data-type="restore" href="<?='<?='?>$this->createUrl('<?=strtolower($this->modelClass)?>/restore', array('id' => $data->id))?>">Восстановить</a>
            <a  class="btn danger" data-type="delete"  href="<?='<?='?>$this->createUrl('<?=strtolower($this->modelClass)?>/delete', array('id'  => $data->id,'ajax' => true))?>" data-title="<?='<?='?>CHtml::encode($data->title)?>" data-id="<?='<?='?>$data->id?>">Удалить</a>
        </p>        
        <?=$date?>
