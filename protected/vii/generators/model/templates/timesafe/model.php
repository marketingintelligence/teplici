...
<?php foreach($relations as $name=>$relation): ?>
			...
			<?php echo "'$name' => $relation,\n"; ?><? if(strstr($relation,'self::HAS_MANY')):?><?php echo "\t\t\t'{$name}Count' => ".str_replace('self::HAS_MANY','self::STAT',$relation).",\n"; ?>
<? endif; ?>
<?php endforeach; ?>
<? if($parent_id):?>
...
foreach($columns as $name=>$column)
{
	if(stripos($column->name,'_at')!==false || stripos($column->name,'date')!==false)
	{
        ...
        echo "if (strstr(\$this->$name,'-')) {
            ...
            \$minute = \$hour = 0;
            if(isset(\$_POST['_time']['$name'])){
                \$time = explode(':',\$_POST['_time']['$name']);
                \$hour = (int)\$time[0];
                \$minute = (int)\$time[1];
            }
            \$this->$name=mktime( \$hour, \$minute, 0, \$date[1], \$date[0], \$date[2] );";
        echo "\n\t}\n";
	}
}
?>
