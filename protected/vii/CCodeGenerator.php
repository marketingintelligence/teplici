...
class CCodeGenerator extends Controller
{
	...
	{
		return 'Vii - Генератор "'.ucfirst($this->id).'"';
	}
	...
	public function actionTableInfo($table)
	{
		$table  = Yii::app()->db->getSchema()->getTable($table);
		$modelClass=Yii::import($this->codeModel,true);
		$model=new $modelClass;
		echo json_encode($model->generateLabels($table));
	}
	public function actionModelInfo($model)
	{
		$table = CActiveRecord::model($model)->tableName();		
		$table  = Yii::app()->db->getSchema()->getTable($table);
		$labels = array();		
		foreach ($table->columns as $key=>$column){		
			if(strstr($column->name,'_at')){
				$labels[$key]='date';
			}elseif(strstr($column->name,'_id')){
				$labels[$key]='parent';
			}elseif(strstr($column->name,'title')||strstr($column->name,'content')){
				$labels[$key]='text';
			}elseif($column->type=='tinyint(1)' || strstr($column->name,'status')){
				$labels[$key]='checkbox';
			}elseif(!strstr($column->name,'image')){
				$labels[$key]='';
			}
		}		
	...
	{
		Yii::import('vii.components.TextDiff');
