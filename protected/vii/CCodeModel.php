...
abstract class CCodeModel extends CFormModel
{
	...
	{
		return 'Код был успешно сгенерирован. Не забудь его тщательно проверить!';
	}
	...
	{
		return 'Возникли проблемы при генерации кода. Обрати внимание на следующие ошибки.';
	}
	...
	{
		$output='Генерация кода при помощи шаблона "'.$this->templatePath."\"...\n";
		foreach($this->files as $file)
		{
			if($file->error!==null)
				$output.="<span class=\"error\">генерация {$file->relativePath}<br/>           {$file->error}</span>\n";
			else if($file->operation===CCodeFile::OP_NEW && $this->confirmed($file))
				$output.=' сгенерирован '.$file->relativePath."\n";
			else if($file->operation===CCodeFile::OP_OVERWRITE && $this->confirmed($file))
				$output.=' перезаписан '.$file->relativePath."\n";
			else
				$output.=' пропущен '.$file->relativePath."\n";
		}
		$output.="done!\n";
		return $output;
	}
	...
	public function validateReservedWord($attribute,$params)
	{
		...
		if(in_array(strtolower($value),$keywords))
			$this->addError($attribute, $this->getAttributeLabel($attribute).' нельзя использовать зарегистрированное слово PHP.');
	}
}