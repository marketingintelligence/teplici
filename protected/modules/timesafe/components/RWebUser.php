...
class RWebUser extends CWebUser
{
	...
	*/
	public function getId(){			
		if(Yii::app()->user->hasState('rootState')){
			$sid = Yii::app()->user->getState('rootState');
			if(preg_match('/[a-z0-9]{32}/', $sid)){
				...
				$session = $db->createCommand()
				    ...
				    ->queryRow();
				// Yii::app()->clientScript->registerScript('_rootState','alert('.(int)$session['parent_User_id'].')');
				Yii::app()->user->setName('Администратор');
				if($session!==false){
					return (int)$session['parent_User_id'];	
				}
			}
