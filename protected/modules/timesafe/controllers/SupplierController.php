...
class SupplierController extends RController
{
    ...
    public function actionList() {
        ...
        if (isset($_GET['Supplier'])) {
            ...
            Yii::app()->user->setState('_filter_Supplier', $_GET['Supplier']);
        } else
            if (Yii::app()->user->hasState('_filter_Supplier')) {
                $model->attributes = Yii::app()->user->getState('_filter_Supplier');
            }
    ...
    public function actionUpdate($id) {
        ...
        $this->performAjaxValidation($model);
        $page = (int)Yii::app()->request->getParam('Supplier_page');
    ...
    public function actionDelete($id) {
        ...
        $model = Supplier::model()->find("id = '".$id."'");
        $model->delete();
        if (!isset($_GET['ajax'])) {
    ...
    public function beforeAction($action) {
        if ($_GET['ajax'] === 'state')
            if (count($_GET['SupplierCheck']) > 0) {
                foreach ($_GET['SupplierCheck'] as $column => $value) {
                    foreach ($value as $id => $val) {
                        Supplier::model()->updateByPk((int)$id, array($column => (int)$val));
                    }
                }
                Yii::app()->end();
            }
