<div class="row">
    <div class="offset">
        ...
        <p class="pull-right">
            <?=CHtml::checkbox('PartnersCheck[status_int][' . $data->id . ']', $data->status_int, array('class' => 'toggle-on-check'))?>
            <span class="label label-info"><i class="icon-eye-open"></i> Видимость</span>
        </p>
        <div style = "display:inline-block; vertical-align: top; margin-right:10px;">
            <? $img = json_decode($data->image, true); ?>
            <img width = "100" src = "/upload/Partners/tm/<?=$img[0]?>">
        </div>
        <div style="display: inline-block">
            <? $img = json_decode($data->image, true); ?>
            <h3 style = "margin-top:5px;">
                <a target = "_blank" href = "/#partners_ids"><?=$data->name_text?></a>
            </h3>
