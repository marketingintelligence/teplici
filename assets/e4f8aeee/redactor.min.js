...
(function(b) {
	...
	a.prototype = {
		...
		init: function() {
			b("script").each(function(d, e) {
				if (e.src) {
					if (e.src.match(/\/redactor\.min\.js.*$/)) {
						this.opts.path = e.src.replace(/redactor\.min\.js(\?.*)?$/, "")
					} else {
						if (e.src.match(/\/redactor\.js.*$/)) {
							this.opts.path = e.src.replace(/redactor\.js(\?.*)?$/, "")
						}
					}
				}
			}.bind2(this));
			...
			if (this.opts.air) {
				...
				this.air = b('<div id="imp_redactor_air_' + this.editorID + '" class="redactor_air" style="display: none;"></div>')
			}
			var c = [];
		...
		start: function() {
			...
			this.buildResizer();
			this.enableAir();
			b(this.doc).bind("paste", function(c) {
				setTimeout(function() {
					var d = b('<span id="pastemarkerend">&nbsp;</span>');
					this.insertNodeAtCaret(d.get(0));
					this.clean()
				}.bind2(this), 200)
			}.bind2(this));
			b(this.doc).keypress(function(h) {
				var f = h.keyCode || h.which;
				if (navigator.userAgent.indexOf("Safari")) {
					if (!h.shiftKey && !h.ctrlKey && !h.metaKey && f == 13) {
						if (h.preventDefault) {
							h.preventDefault()
						}
						var d = b("<span></span>");
				...
				if (f == 13 && h.shiftKey) {
					if (this.getParentNodeName() == "BODY") {
						...
						var g = b("<p>&nbsp;</p>");
						this.insertNodeAtCaret(g.get(0));
						return false
					} else {
						return true
					}
			...
			b(this.doc).keyup(function(f) {
				var c = f.keyCode || f.which;
				if (c == 8 || c == 46) {
					if (b(this.doc.body).html() == "") {
						if (f.preventDefault) {
							f.preventDefault()
						}
						var d = b("<p>&nbsp;</p>").get(0);
						b(this.doc.body).append(d);
						this.setFocusNode(d);
						return false
					}
				}
				if (c == 13 && !f.shiftKey && !f.ctrlKey && !f.metaKey) {
					if (this.getParentNodeName() == "BODY") {
						if (f.preventDefault) {
							f.preventDefault()
						}
		...
		build: function() {
			this.$box = b('<div id="redactor_box_' + this.editorID + '" class="redactor_box"></div>');
			this.$frame = b('<iframe frameborder="0" scrolling="auto" id="redactor_frame_' + this.editorID + '" style="height: ' + this.height + ';" class="redactor_frame"></iframe>');
			this.$el.hide();
		...
		enableAir: function() {
			...
			b("#imp_redactor_air_" + this.editorID).hide();
			b(this.doc).bind("textselect", this.editorID, function(f) {
				var c = b("#imp_redactor_air_" + this.editorID).width();
				var g = this.$frame.width();
				var d = g - f.clientX;
				if (d < c) {
					f.clientX = f.clientX - c
				}
				b("#imp_redactor_air_" + this.editorID).css({
					left: f.clientX + "px",
					top: (f.clientY + 8) + "px"
				}).show()
			}.bind2(this));
			b(this.doc).bind("textunselect", this.editorID, function() {
				b("#imp_redactor_air_" + this.editorID).hide()
			}.bind2(this))
		},
		write: function(c) {
		...
		setDoc: function(d) {
			var c = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">\n';
		...
		docObserve: function() {
			b(this.doc.body).find("img").click(function(c) {
				this.imageEdit(c)
			}.bind2(this));
			b(this.doc.body).find("table").click(function(c) {
				this.tableObserver(c)
			}.bind2(this));
			b(this.doc.body).find(".redactor_file_link").click(function(c) {
				this.fileEdit(c)
			}.bind2(this))
		},
		formSubmit: function() {
		...
		execCommand: function(c, f) {
			if (this.opts.visual && this.doc) {
				...
				this.syncCodeToTextarea();
				if (this.opts.air) {
					b("#imp_redactor_air_" + this.frameID).hide()
				}
			}
		},
		clean: function(c) {
			var d = this.getCodeEditor();
			if (c !== false) {
				d = d.replace(/<span id="pastemarkerend">&nbsp;<\/span>/, "#marker#")
			}
			d = this.formating(d);
			d = d.replace(/(<\!\-\-([\w\W]*?)\-\->)/ig, "");
			d = this._clean(d);
			d = d.replace(/<div(.*?)>/gi, "<p$1>");
			d = d.replace(/<\/div>/, "</p>");
			d = d.replace(/ lang="([\w\W]*?)"/gi, "");
			if (this.opts.removeClasses) {
				d = d.replace(/ class="([\w\W]*?)"/gi, "")
			} else {
				d = this._cleanClasses(d)
			}
			if (this.opts.removeStyles) {
				d = d.replace(/ style="([\w\W]*?)"/gi, "")
			} else {
				d = this._cleanStyles(d)
			}
			d = d.replace(/<a name="(.*?)">([\w\W]*?)<\/a>/gi, "");
			d = d.replace(/\&nbsp;\&nbsp;\&nbsp;/gi, " ");
			d = d.replace(/\&nbsp;\&nbsp;/gi, " ");
			d = d.replace(/\s*style="\s*"/gi, "");
			d = d.replace(/\<span>&nbsp;<\/span>/gi, "");
			d = d.replace(/<span>([\w\W]*?)<\/span>/gi, "$1");
			d = this._clean(d);
			d = this.formating(d);
			if (c !== false) {
				d = d.replace(/#marker#/, '<span id="pastemarkerend">&nbsp;</span>')
			}
			this.setCodeEditor(d);
			if (c !== false) {
				var e = b(this.doc.body).find("#pastemarkerend").get(0);
				this.setFocusNode(e)
			}
		},
		_cleanClasses: function(c) {
		...
		_clean: function(c) {
			return c.replace(/<(?!\s*\/?(span|label|a|br|p|b|i|del|strike|img|video|audio|iframe|object|embed|param|blockquote|mark|cite|small|ul|ol|li|hr|dl|dt|dd|sup|sub|big|pre|code|figure|figcaption|strong|em|table|tr|td|th|tbody|thead|tfoot|h1|h2|h3|h4|h5|h6)\b)[^>]+>/gi, "")
		},
		formating: function(e) {
			if (b.browser.msie) {
				e = e.replace(/< *(\/ *)?(\w+)/g, function(h) {
					return h.toLowerCase()
				});
				e = e.replace(/style="(.*?)"/g, function(h) {
					return h.toLowerCase()
				});
				e = e.replace(/ jQuery(.*?)=\"(.*?)\"/gi, "")
			}
			if (b.browser.mozilla) {
				e = this.convertSpan(e)
			}
			e = e.replace(/\<font([\w\W]*?)color="(.*?)">([\w\W]*?)\<\/font\>/gi, '<span style="color: $2;">$3</span>');
			e = e.replace(/\<font([\w\W]*?)>([\w\W]*?)\<\/font\>/gi, "<span$1>$2</span>");
			e = e.replace(/\<p><span>([\w\W]*?)<\/span><\/p>/gi, "<p>$1</p>");
			e = e.replace(/<span>([\w\W]*?)<\/span>/gi, "$1");
			e = e.replace(/ class="Apple-style-span"/gi, "");
			e = e.replace(/ class="Apple-tab-span"/gi, "");
			e = e.replace(/<p><p>/g, "<p>");
			e = e.replace(/<\/p><\/p>/g, "</p>");
			e = e.replace(/<hr(.*?)>/g, "<hr>");
			e = e.replace(/<p>&nbsp;/g, "<p>");
			e = e.replace(/<p><ul>/g, "<ul>");
			e = e.replace(/<p><ol>/g, "<ol>");
			e = e.replace(/<\/ul><\/p>/g, "</ul>");
			e = e.replace(/<\/ol><\/p>/g, "</ol>");
			e = e.replace(/[\t]*/g, "");
			e = e.replace(/\n\s*\n/g, "\n");
			e = e.replace(/^[\s\n]*/, "");
			e = e.replace(/[\s\n]*$/, "");
			var g = ["<pre></pre>", "<blockquote></blockquote>", "<em></em>", "<b></b>", "<ul></ul>", "<ol></ol>", "<li></li>", "<table></table>", "<tr></tr>", "<span><span>", "<span>&nbsp;<span>", "<p> </p>", "<p></p>", "<p>&nbsp;</p>", "<p><br></p>", "<div></div>"];
			for (i = 0; i < g.length; ++i) {
				var c = g[i];
				e = e.replace(new RegExp(c, "gi"), "")
			}
			var f = "\r\n";
			var g = ["<form", "<fieldset", "<legend", "<object", "<embed", "<select", "<option", "<input", "<textarea", "<br>", "<br />", "<pre", "<blockquote", "<ul", "<ol", "<li", "<dl", "<dt", "<dd", "<!--", "<table", "<thead", "<tbody", "<caption", "</caption>", "<th", "<tr", "<td", "<figure"];
			for (i = 0; i < g.length; ++i) {
				var c = g[i];
				e = e.replace(new RegExp(c, "gi"), f + c)
			}
			var d = ["</p>", "</div>", "</ul>", "</ol>", "</h1>", "</h2>", "</h3>", "</h4>", "</h5>", "</h6>", "</dl>", "</dt>", "</dd>", "</form>", "</blockquote>", "</pre>", "</legend>", "</fieldset>", "</object>", "</embed>", "</textarea>", "</select>", "</option>", "</table>", "</thead>", "</tbody>", "</tr>", "</td>", "</th>", "</figure>"];
			for (i = 0; i < d.length; ++i) {
				var c = d[i];
				e = e.replace(new RegExp(c, "gi"), c + f)
			}
			e = e.replace(/<li/g, "\t<li");
			e = e.replace(/<tr/g, "\t<tr");
			e = e.replace(/<td/g, "\t\t<td");
		...
		buildToolbar: function() {
			...
			b.each(RTOOLBAR, function(k, n) {
				var l = b("<li>");
				if (k == "fullscreen") {
					b(l).addClass("redactor_toolbar_right")
				}
				var h = b('<a href="javascript:void(null);" title="' + n.title + '" class="redactor_btn_' + k + '"><span>&nbsp;</span></a>');
				if (typeof(n.func) == "undefined") {
					h.click(function() {
						this.execCommand(n.exec, k)
					}.bind2(this))
				} else {
					if (n.func != "show") {
						h.click(function(o) {
				...
				if (k == "backcolor" || k == "fontcolor" || typeof(n.dropdown) != "undefined") {
					var m = b('<div class="redactor_dropdown" style="display: none;">');
					if (k == "backcolor" || k == "fontcolor") {
						if (k == "backcolor") {
							if (b.browser.msie) {
								var e = "BackColor"
							} else {
						...
						b(m).width(210);
						var f = this.opts.colors.length;
						for (var d = 0; d < f; ++d) {
							var c = this.opts.colors[d];
							...
							var g = this;
							b(j).click(function() {
								var o = b(this).attr("rel");
								g.execCommand(e, o)
							})
						}
					} else {
						b.each(n.dropdown, function(o, q) {
							if (typeof(q.style) == "undefined") {
								q.style = ""
							}
							if (q.name == "separator") {
								var p = b('<a class="redactor_separator_drop">')
							} else {
								var p = b('<a href="javascript:void(null);" style="' + q.style + '">' + q.title + "</a>");
								if (typeof(q.func) == "undefined") {
									b(p).click(function() {
										this.execCommand(q.exec, o)
									}.bind2(this))
								} else {
				...
				if (k == "backcolor" || k == "fontcolor" || typeof(n.dropdown) != "undefined") {
					this.hdlHideDropDown = function(o) {
						this.hideDropDown(o, m, k)
					}.bind2(this);
					this.hdlShowDropDown = function(o) {
						this.showDropDown(o, m, k)
					}.bind2(this);
					h.click(this.hdlShowDropDown);
					b(document).click(this.hdlHideDropDown)
				}
				b(l).append(h);
				this.$toolbar.append(l);
				if (typeof(n.separator) != "undefined") {
					var l = b('<li class="separator"></li>');
					this.$toolbar.append(l)
				}
			}.bind2(this));
			b(this.doc).click(function() {
				this.hideAllDropDown()
			}.bind2(this))
		},
		showDropDown: function(h, j, d) {
			this.hideAllDropDown();
			this.setBtnActive(d);
			this.getBtn(d).addClass("dropact");
			var g = this.getBtn(d).position().left;
			if (this.opts.air) {
				var c = this.air.position().left;
				var f = this.air.position().top;
				g += c;
				b(j).css("top", f + 28)
			}
			b(j).css("left", g + "px").show()
		},
		...
		hideDropDown: function(d, f, c) {
			...
			b(document).unbind("click", this.hdlHideDropDown);
			b(this.doc).unbind("click", this.hdlHideDropDown)
		},
		fullscreen: function() {
		...
		saveSelection: function() {
			if (window.getSelection) {
				...
				if (document.selection && document.selection.createRange) {
					this.cursorPosition = this.getSelection()
				}
			}
		...
		restoreSelection: function() {
			if (this.cursorPosition) {
				if (window.getSelection) {
					...
					if (document.selection && this.cursorPosition.select) {
		...
		getParentNodeName: function() {
			if (window.getSelection) {
				return this.getSelection().getRangeAt(0).startContainer.parentNode.nodeName
			} else {
				if (document.selection) {
					return this.getSelection().parentElement().nodeName
				}
			}
		},
		...
		getParentNodeID: function() {
			if (window.getSelection) {
				return this.getSelection().getRangeAt(0).startContainer.parentNode.id
			} else {
				if (document.selection) {
					return this.getSelection().parentElement().id
				}
			}
		},
		...
		insertTable: function() {
			var h = b("#redactor_table_rows").val();
			var e = b("#redactor_table_columns").val();
			var k = b("<div></div>");
			var d = Math.floor(Math.random() * 99999);
			var g = b('<table id="table' + d + '"><tbody></tbody></table>');
			for (i = 0; i < h; i++) {
				var j = b("<tr></tr>");
				for (z = 0; z < e; z++) {
					var f = b("<td>&nbsp;</td>");
			...
			redactorActive.execCommand("inserthtml", c);
			this.enableObjects();
			this.docObserve();
			this.modalClose();
			$table = b(this.doc).find("body").find("#table" + d)
		},
		tableObserver: function(c) {
			$table = b(c.target).parents("table");
			$table_tr = $table.find("tr");
			$table_td = $table.find("td");
			$table_td.removeClass("current");
			$tbody = b(c.target).parents("tbody");
			$thead = b($table).find("thead");
			$current_td = b(c.target);
			$current_td.addClass("current");
			$current_tr = b(c.target).parents("tr")
		},
		...
		deleteColumn: function() {
			var c = b($current_td).get(0).cellIndex;
			b($table).find("tr").each(function() {
				b(this).find("td").eq(c).remove()
			})
		},
		addHead: function() {
			if (b($table).find("thead").size() != 0) {
				this.deleteHead()
			} else {
				var c = b($table).find("tr").first().clone();
				c.find("td").html("&nbsp;");
				$thead = b("<thead></thead>");
				$thead.append(c);
				b($table).prepend($thead)
			}
		},
		deleteHead: function() {
			b($thead).remove();
		...
		insertRow: function(c) {
			var d = b($current_tr).clone();
			d.find("td").html("&nbsp;");
			if (c == "after") {
				b($current_tr).after(d)
			} else {
				b($current_tr).before(d)
			}
		},
		insertColumn: function(d) {
			var c = 0;
			$current_td.addClass("current");
			$current_tr.find("td").each(function(e, f) {
				if (b(f).hasClass("current")) {
					c = e
				}
			});
			$table_tr.each(function(e, f) {
		...
		showFile: function() {
			...
			var c = function() {
					var d = "";
					if (this.opts.fileUploadFunction) {
						d = this.opts.fileUploadFunction()
					}
					b("#redactor_file").dragupload({
						url: this.opts.fileUpload + d,
						success: function(e) {
			...
			redactorActive = this;
			this.modalInit(RLANG.file, this.opts.path + "plugins/file.html?r", 500, 380, c)
		},
		fileUploadCallback: function(c) {
			if (b.browser.msie) {
				b(redactorActive.doc.getElementById("span" + redactorActive.spanid)).after(c);
				b(redactorActive.doc.getElementById("span" + redactorActive.spanid)).remove()
			} else {
				redactorActive.execCommand("inserthtml", c)
			}
			this.modalClose();
		...
		fileEdit: function(g) {
			...
			var c = function() {
					b("#file").val(b(d).text());
					b("#redactorFileDeleteBtn").click(function() {
						this.fileDelete(d, f)
					}.bind2(this));
					b("#redactorFileDownloadBtn").click(function() {
						this.fileDownload(d, f)
					}.bind2(this))
				}.bind2(this);
			redactorActive = this;
			this.modalInit(RLANG.file, this.opts.path + "plugins/file_edit.html", 400, 200, c)
		},
		fileDelete: function(c, d) {
			b(c).remove();
			b.get(this.opts.fileDelete + d);
			redactorActive.$frame.get(0).contentWindow.focus();
			this.modalClose()
		},
		fileDownload: function(c, d) {
			top.location.href = this.opts.fileDownload + d
		},
		imageEdit: function(d) {
			var c = function() {
					var e = b(d.target);
					b("#redactor_file_alt").val(e.attr("alt"));
					b("#redactor_form_image_align").val(e.css("float"));
					b("#redactor_image_edit_delete").click(function() {
						this.deleteImage(d.target)
					}.bind2(this));
					b("#redactorSaveBtn").click(function() {
						this.imageSave(d.target)
					}.bind2(this))
				}.bind2(this);
			redactorActive = this;
			this.modalInit(RLANG.image, this.opts.path + "plugins/image_edit.html", 425, 290, c)
		},
		imageSave: function(c) {
			b(c).attr("alt", b("#redactor_file_alt").val());
			var d = b("#redactor_form_image_align").val();
			if (d == "left") {
				b(c).css({
					"float": "left",
		...
		showImage: function() {
			...
			var c = function() {
					if (this.opts.imageGetJson !== false) {
						b.getJSON(this.opts.imageGetJson, function(e) {
							b.each(e, function(g, h) {
								var f = b('<img src="' + h.thumb + '" rel="' + h.image + '">');
								f.click(function() {
									redactorActive.imageSetThumb(b(this).attr("rel"))
								});
					...
					var d = "";
					if (this.opts.imageUploadFunction) {
						d = this.opts.imageUploadFunction()
					}
					b("#redactor_file").dragupload({
						url: this.opts.imageUpload + d,
						success: function(e) {
							this.imageUploadCallback(e)
						}.bind2(this)
					});
					this.uploadInit("redactor_file", {
						auto: true,
					...
					b("#redactorUploadBtn").click(this.imageUploadCallbackLink)
				}.bind2(this);
			redactorActive = this;
			this.modalInit(RLANG.image, this.opts.path + "plugins/image.html", 570, 410, c)
		},
		imageSetThumb: function(c) {
			this._imageSet('<img alt="" src="' + c + '" />')
		},
		imageUploadCallbackLink: function() {
			if (b("#redactor_file_link").val() != "") {
				var c = '<img src="' + b("#redactor_file_link").val() + '">';
				redactorActive._imageSet(c)
			} else {
				this.modalClose()
			}
		},
		...
		showLink: function() {
			...
			var c = function() {
					var f = this.getSelection();
					if (b.browser.msie) {
						var d = f.htmlText.match(/href="(.*?)"/gi);
						if (d != null) {
							d = new String(d);
							d = d.replace(/href="(.*?)"/gi, "$1")
						}
						var h = f.text;
						if (d != null) {
							var e = d
						...
						if (f.anchorNode.parentNode.tagName == "A") {
							var e = f.anchorNode.parentNode.href;
							var h = f.anchorNode.parentNode.text;
							if (f.toString() == "") {
								this.insert_link_node = f.anchorNode.parentNode
							}
						} else {
							var h = f.toString();
							var e = ""
					...
					b("#redactor_link_url").val(e).focus();
					b("#redactor_link_text").val(h)
				}.bind2(this);
			this.modalInit(RLANG.link, this.opts.path + "plugins/link.html", 400, 260, c)
		},
		insertLink: function() {
			var f = b("#redactor_link_text").val();
			if (f == "") {
				return false
			}
			if (b("#redactor_link_id_url").get(0).checked) {
				var d = ""
			} else {
				var d = "mailto:"
			}
			var c = '<a href="' + d + b("#redactor_link_url").val() + '">' + f + "</a> ";
			if (c) {
				if (this.insert_link_node) {
					b(this.insert_link_node).text(f);
					b(this.insert_link_node).attr("href", b("#redactor_link_url").val());
					this.modalClose()
				} else {
					if (b.browser.msie) {
						b(redactorActive.doc.getElementById("span" + redactorActive.spanid)).after(c);
						...
						redactorActive.insertNodeAtCaret(b(c).get(0));
						var e = b("<span></span>").get(0);
						redactorActive.insertNodeAtCaret(e);
						redactorActive.setFocusNode(e)
					}
				}
			}
			this.modalClose()
		...
		insertVideo: function() {
			...
			if (redactorActive.opts.visual) {
				if (c.search("iframe")) {} else {
					c = '<p class="redactor_video_box">' + c + "</p>"
				}
			}
			if (b.browser.msie) {
				b(redactorActive.doc.getElementById("span" + redactorActive.spanid)).after(c);
		...
		buildResizer: function() {
			...
			this.$resizer_box = b("<div>").addClass("redactor_resizer");
			this.$resizer = b("<div>");
			this.$resizer_box.append(this.$resizer);
			this.$box.append(this.$resizer_box);
			this.$resizer.mousedown(function(c) {
				this.initResize(c)
			}.bind2(this))
		},
		initResize: function(c) {
			...
			this.stopResizeHdl = function(d) {
				this.stopResize(d)
			}.bind2(this);
			this.startResizeHdl = function(d) {
				this.startResize(d)
			}.bind2(this);
			this.resizeHdl = function(d) {
				this.resize(d)
			}.bind2(this);
		...
		setCodeEditor: function(c) {
			b(this.doc.body).html(c);
			this.docObserve()
		},
		setCodeTextarea: function(c) {
			this.$el.val(c)
		},
		getCodeEditor: function() {
			return b(this.doc.body).html()
		},
		getCodeTextarea: function(c) {
			if (c !== false) {
				this.clean(false);
				this.syncCodeToTextarea()
			}
			return b.trim(this.$el.val())
		},
		syncCodeToTextarea: function() {
			if (this.opts.visual) {
				this.setCodeTextarea(this.getCodeEditor())
			}
		},
		...
		handler: function() {
			...
			b.ajax({
				...
				type: "post",
				data: "redactor=" + escape(encodeURIComponent(c)),
				success: function(d) {
					this.setCodeEditor(d);
					this.syncCodeToTextarea()
				}.bind2(this)
		...
		autoSave: function() {
			...
			setInterval(function() {
				b.post(this.opts.autosave, {
					data: this.getCodeEditor()
				})
			}.bind2(this), this.opts.interval * 1000)
		},
		modalInit: function(h, e, g, d, f, c) {
			...
			if (this.opts.overlay) {
				...
				b("#redactor_modal_overlay").click(function() {
					this.modalClose()
				}.bind2(this))
			}
			...
			b("#redactor_modal_close").click(function() {
				this.modalClose()
			}.bind2(this));
			b(document).keyup(function(j) {
				if (j.keyCode == 27) {
					this.modalClose()
				}
			}.bind2(this));
			b(this.doc).keyup(function(j) {
				if (j.keyCode == 27) {
					this.modalClose()
				}
			}.bind2(this));
			b.ajax({
				...
				url: e,
				success: function(j) {
					b.each(RLANG, function(k, m) {
						var l = new RegExp("%RLANG." + k + "%", "gi");
						j = j.replace(l, m)
					});
					b("#redactor_modal_inner").html(j);
					...
					if (c === true) {
						b("#imp_redactor_table_box").height(d - b("#redactor_modal_header").outerHeight() - 130).css("overflow", "auto")
					}
		...
		modalClose: function() {
			b("#redactor_modal_close").unbind("click", function() {
				this.modalClose()
			}.bind2(this));
			b("#redactor_modal").fadeOut("fast", function() {
				b("#redactor_modal_inner").html("");
				if (this.opts.overlay) {
					b("#redactor_modal_overlay").hide();
					b("#redactor_modal_overlay").unbind("click", function() {
						this.modalClose()
					}.bind2(this))
				}
				b(document).unbind("keyup", function(c) {
					if (c.keyCode == 27) {
						this.modalClose()
					}
				}.bind2(this));
				b(this.doc).unbind("keyup", function(c) {
					if (c.keyCode == 27) {
						this.modalClose()
					}
				}.bind2(this))
		...
		uploadInit: function(d, c) {
			...
			if (this.uploadOptions.auto) {
				b(this.uploadOptions.input).change(function() {
				...
				if (this.uploadOptions.trigger) {
					b("#" + this.uploadOptions.trigger).click(function() {
						this.uploadSubmit()
					}.bind2(this))
				}
			}
	...
	Function.prototype.bind2 = function(d) {
		var e = this;
...
function showRedactorTabs(b, a) {
	$("#redactor_tabs a").removeClass("redactor_tabs_act");
	$(b).addClass("redactor_tabs_act");
	$(".redactor_tabs").hide();
	$("#redactor_tabs" + a).show()
}(function(b) {
	...
	a.prototype = {
		init: function() {
			if (!b.browser.opera && !b.browser.msie) {
				...
				this.dropareabox.get(0).ondrop = function(e) {
					e.preventDefault();
...
(function(a) {
	a.event.special.textselect = {
		...
		getSelectedText: function(b) {
			var d = "";
			var c = a("#redactor_frame_" + b).get(0);
			if (c.contentWindow.getSelection) {
				d = c.contentWindow.getSelection()
			} else {
				if (c.contentWindow.document.getSelection) {
					d = c.contentWindow.document.getSelection()
				} else {
					if (c.contentWindow.document.selection) {
						d = c.contentWindow.document.selection.createRange().text
					}
				}
			}
			return d
